...
# Skip bundled Chromium download; system Chromium is installed in the final stage
...
RUN apt-get update -qq && \
    ...
    nodejs `# js runtime` \
    chromium `# PDF generation via Grover/Puppeteer` && \
...
RUN useradd rails --create-home --shell /bin/bash && \
    mkdir -p db/sphinx/production && \
    chown -R rails:rails db log storage tmp

# https://serverfault.com/a/984599
RUN mkdir -p /srv/public /srv/sphinx \
 && chown -R rails:rails /srv/public /srv/sphinx
